...
//   var BOUNDARY = 0.45;
    ...
    var PARTICLE_MULTIPLIER = 1 / 3000;              // particle count scalar (completely arbitrary--this values looks nice)
    var PARTICLE_REDUCTION = 0.75;            // reduce particle count to this much of normal for mobile devices
    ...
    var buildGrid = function (data, callback) {
      ...
      // let λ0 = header.lo1, φ0 = header.la2;
      var λ0 = header.lo1;
      //gfs, vs hrrr and nam
      var φ0 = params.modType==='global'?header.la1: header.la2;
    ...
    var animate = function (bounds, field) {
      ...
      var buckets = windy.colorStyles.map(function () {
        return [];
      });
        ...
        particles.forEach(function (particle) {
          ...
          if (m === null) {
            ...
            if (field(xt, yt)[2] !== null) {
              ...
              particle.xt = xt;
              particle.yt = yt;
              buckets[windy.colorStyles.indexFor(m)].push(particle);
            }
            else {
      ...
      function draw() {
        ...
        buckets.forEach(function (bucket, i) {
          if (bucket.length > 0) {
            g.beginPath();
            g.strokeStyle = windy.colorStyles[i];
    ...
    function windIntensityColorScale(step, maxWind) {
      ...
      var result = [
        /* blue to red */
        "rgba(" + hexToR('#1899ff') + ", " + hexToG('#1899ff') + ", " + hexToB('#1899ff') + ", " + 1 + ")",
        "rgba(" + hexToR('#b8b8ff') + ", " + hexToG('#b8b8ff') + ", " + hexToB('#b8b8ff') + ", " + 1 + ")",
        "rgba(" + hexToR('#ffbadf') + ", " + hexToG('#ffbadf') + ", " + hexToB('#ffbadf') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff9e98') + ", " + hexToG('#ff9e98') + ", " + hexToB('#ff9e98') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff8867') + ", " + hexToG('#ff8867') + ", " + hexToB('#ff8867') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff7348') + ", " + hexToG('#ff7348') + ", " + hexToB('#ff7348') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff612d') + ", " + hexToG('#ff612d') + ", " + hexToB('#ff612d') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff5017') + ", " + hexToG('#ff5017') + ", " + hexToB('#ff5017') + ", " + 1 + ")",
        "rgba(" + hexToR('#ff3705') + ", " + hexToG('#ff3705') + ", " + hexToB('#ff3705') + ", " + 1 + ")",
      ...
      */
      ...
      result.maxSpeedForIndex = function (i) {
        return Math.min(Math.round((i+1) / (result.length - 1) * maxWind * 10) / 10, maxWind);
      };
      return result;
    }
