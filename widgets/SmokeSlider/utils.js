...
define(['dojo/_base/html', 'dojo/dom-geometry', 'dojo/_base/array', "esri/layers/TimeInfo", 'jimu/utils', 'esri/TimeExtent', 'libs/storejs/store', 'moment/moment'], function (html, domGeometry, array, esriTimeInfo, jimuUtils, TimeExtent, store, moment) {
  ...
  mo.isLayerEnabledTime = function (layer, layerInfosObj, layerInfo) {
    ...
    if (originLayer.itemProperties && "undefined" !== typeof originLayer.itemProperties.timeAnimation && "undefined" !== typeof originLayer.timeAnimation) {
      usesTime = true;
    }
  ...
  mo.getCalendarTime = function (customTimeConfig, refTime) {
    var time;
    if ("time" === customTimeConfig.timeMode || "min" === customTimeConfig.timeMode || "max" === customTimeConfig.timeMode) {
      //1 time mode
  ...
  mo.isConfigLiveMode = function (config) {
    ...
    if (config && config.customTimeConfig && config.customTimeConfig.startTime && config.customTimeConfig.startTime.timeConfig && config.customTimeConfig.startTime.timeConfig.timeMode) {
      startTime = mo.isLiveTimeMode(config.customTimeConfig.startTime.timeConfig.timeMode);
    }
    if (config && config.customTimeConfig && config.customTimeConfig.endTime && config.customTimeConfig.endTime.timeConfig && config.customTimeConfig.endTime.timeConfig.timeMode) {
  ...
  mo.isLiveTimeMode = function (mode) {
    if (mode && ("now" === mode || "today" === mode || "max" === mode || "min" === mode)) {
      return true;
    }
  ...
  mo.getFullTimeExtent = function (timeExtents, noRound) {
    ...
    fullTimeExtent.endTime = new Date(fullTimeExtent.endTime.getFullYear(), fullTimeExtent.endTime.getMonth(), fullTimeExtent.endTime.getDate(), fullTimeExtent.endTime.getHours(), fullTimeExtent.endTime.getMinutes() + roundTime, 0, 0);
  ...
  mo.getAutoRefreshTime = function (config) {
    ...
    if (config && config.autoRefresh && true === config.autoRefresh.isAutoRefresh && config.autoRefresh.interval && config.autoRefresh.unit) {
