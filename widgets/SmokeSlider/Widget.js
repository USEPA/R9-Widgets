...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/array', 'dojo/_base/fx', "jimu/WidgetManager", 'dojo/dnd/move', 'dojo/on', 'dojo/query', 'dojo/Deferred', 'dojo/dom-geometry', 'jimu/LayerInfos/LayerInfos', 'jimu/LayerStructure', 'jimu/LayerNode', 'jimu/BaseWidget', 'esri/TimeExtent', 'esri/dijit/TimeSlider', './SpeedMenu', 'jimu/utils', './TimeProcesser', './LayerProcesser', './utils', "./a11y/Widget", 'dijit/Dialog', 'dojo/text!./WindDialog.html'
], function (declare, lang, html, array, baseFx, WidgetManager, Move, on, query, Deferred, domGeometry, LayerInfos, LayerStructure, LayerNode, BaseWidget, TimeExtent, TimeSlider, SpeedMenu, jimuUtils, TimeProcesser, LayerProcesser, utils, a11y, Dialog, windDialogContent) {
  ...
  var clazz = declare([BaseWidget], {
    ...
    onOpen: function onOpen() {
      ...
      var all_layers = layerStructure.getLayerNodes();
      // NDGD_SmokeForecast_v1_9218
      var smoke_node = all_layers.find(function (lyr) {
        return lyr.id.includes('NDGD_SmokeForecast');
      });
      smoke_node.show();
      // AirNowLatestContoursPM25_2250
      this.airQLayer = all_layers.find(function (lyr) {
        return lyr.id.includes('AirNowLatestContours');
      });
      this.airQ_label.innerHTML = this.airQLayer.title;

      this.listeners = [this.airQLayer.on(LayerNode.EVENT_TOOGLE_CHANGE, function () {
        _this._ontoggleAirQ(undefined);
      })];
    ...
    _closeOthersTimeSlider: function _closeOthersTimeSlider() {
    ...
    _openHanlder: function _openHanlder() {
    ...
    onClose: function onClose() {
      ...
      // DW turn off layer
      var layerStructure = LayerStructure.getInstance();
      layerStructure.getNodeById("NDGD_SmokeForecast_v1_9218").hide();
    ...
    _showNoTimeLayer: function _showNoTimeLayer() {
      ...
      return this.getTimeSliderProps().then(lang.hitch(this, function (data) {
        ...
        **************************************/
        ...
        if (false === this.config.isHonorWebMap) {
          ...
          this.timeSlider.createTimeStopsByTimeInterval(timeExtent, timeConfig.interval.number, timeConfig.interval.units);

          if (this._LAST_SWIPE_TIME) {
            this._LAST_SWIPE_TIME = null;
          }
        } else {
          //true === this.config.isHonorWebMap
          ...
          if (props) {
            ...
            if (props.numberOfStops) {
              ...
              this.timeSlider.createTimeStopsByTimeInterval(timeExtent, props.timeStopInterval.interval, props.timeStopInterval.units);
            }
            ...
            if (props.currentTimeExtent) {
              ...
              if (props.thumbCount === 1) {
                this.timeSlider.setThumbIndexes([this.timeProcesser.findClosestThumbIndex(this.timeSlider, props.currentTimeExtent[1])]);
              } else {
            ...
            this.timeSlider.createTimeStopsByTimeInterval(fullTimeExtent, props.timeStopInterval.interval, props.timeStopInterval.units);
          }
        }
        ...
        this._timeHandles.push(on(this.timeSlider, 'time-extent-change', lang.hitch(this, this.onTimeSliderChange)));

        return this.timeSlider;
      }));
    },
    onTimeSliderChange: function onTimeSliderChange(timeExtent) {
    ...
    timeCalendarDataAutoRefresh: function timeCalendarDataAutoRefresh(timeExtent) {
    ...
    _updateBtnsUI: function _updateBtnsUI() {
      ...
      if (!this._playBtnHandler) {
        this._playBtnHandler = this.own(on(this.playBtn, 'click', lang.hitch(this, function (evt) {
          ...
          var slider = this.timeSlider._slider,
              val;
    ...
    updateLayerLabel: function updateLayerLabel() {
    ...
    updateTimeExtentLabel: function updateTimeExtentLabel(timeExtent) {
    ...
    //play btn style
    _setPlayBtnStyleTimer: function _setPlayBtnStyleTimer() {
    ...
    _cleanPlayBtnStyleTimer: function _cleanPlayBtnStyleTimer() {
    ...
    //LayerInfosObj
    _initLayerInfosObj: function _initLayerInfosObj() {
      if (!this._layerInfosDef) {
        ...
        LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function (layerInfosObj) {
          ...
          this.own(on(layerInfosObj, 'layerInfosIsShowInMapChanged', lang.hitch(this, this.layerProcesser._onLayerInfosIsShowInMapChanged)));
          this.own(layerInfosObj.on('layerInfosChanged', lang.hitch(this, this.layerProcesser._onLayerInfosChanged)));
    ...
    _ontoggleAirQ: function _ontoggleAirQ(evt) {
      var airq_check = this.airq_toggle_node;
      if (evt !== undefined) {
        evt.stopPropagation();
