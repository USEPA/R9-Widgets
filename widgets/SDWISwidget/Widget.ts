...
class Widget implements IWidget {
  ...
  private onOpen(): void {
    ...
    this.featureLayer.queryCount(query, (count: number) => {
      this.myNode.innerHTML = `There are currently <b>${count}</b> facilities in the SDWIS feature service.`+`</br><h2 style="text-decoration: underline;">Safe Drinking Water Information System (SDWIS)</h2>`+`The data is directly from the <b>National SDWIS Database</b> and updated on a quarterly basis. The <b><a href="https://epa.maps.arcgis.com/home/item.html?id=107c64d513ad4f1f8b80e4f60ab5a851"target="_blank">GeoPlatform service</a></b> provides information on facilities, public water systems, primacy agencies, administrative contacts, and tribal entities.  The facility symbols are <i>clustered</i> to minimize overlap; zoom in closer to see a facility's true location.`+`</br></br>`+`Detailed information about the <b>SDWIS Federal Reporting Services</b> can be found <b><a href="https://www.epa.gov/ground-water-and-drinking-water/safe-drinking-water-information-system-sdwis-federal-reporting"target="_blank">here.</a></b></br><h2 style="text-decoration: underline;">Enforcement & Compliance History Online (ECHO)</h2>`+`EPA's ECHO website provides details for facilities in your community to assess their compliance with environmental regulations.  The interaction in this widget uses the Public Water System (PWS) ID to search the records.  Check out the ECHO website <a href="https://echo.epa.gov/"target="_blank"><b>here</b></a> for more information and guidance.</br></br>The <i><b>ECHO Detailed System Report</b></i> is linked with the selected facility record and opens the ECHO website details for the associated public water system in a new browser window.`+`</br><h2 style="text-decoration: underline;">Definitions</h2>`+`<b>Facilities - </b>These points represent facilities within a public water system.  The facility types include but are not limited to wells, well heads, treatment plants, sampling stations, valves, transmission mains, pumps, pressure control, etc.  Facilities are identified with Facility ID and Facility Name.  The PWS ID indicates the public water system the selected facility falls under.</br>`+`</br><b>Public Water Systems (PWS) - </b> These points are representative centroids of the facilities that are within their jurisdiction and system.  They do not indicate a site location in any way.  Any points in line with a facility point is coincidental.  The PWS ID and PWS Name provide the unique identification for the PWS. </br> <p style="text-align: left;">&nbsp;</p> <table style=" background-color: #d5e1ef; border-color: #000000; margin-left: auto; margin-right: auto;" width="100%"><tbody><td style="text-align: left; width: 287px;"><b>PWS Contact Information - </b>This section provides contact information for the public water system associated with the selected facility.  This information comes from the Admin Contacts table.</table>`+`</br><table style="height: 98px; background-color: #acc3df; border-color: #000000; margin-left: auto; margin-right: auto;" width="100%"><tbody></tbody><td style="text-align: left; width: 287px;"><b>Regulatory Agency - </b> This section provides information for the regulatory organization associated with and responsible for the selected facility's public water system.  This information comes from the Primacy Agency table.`+`</br></td></table>`;
  ...
  private _clickHandler() {
    ...
    this.myNode.innerHTML = `<p style="font-size:16px"><b>Public Water System (PWS)</b></p>`+ `<b><p style="font-size:14px">Name: </b>` + (facility.attributes.Fac_PWS_Name ? facility.attributes.Fac_PWS_Name : 'Not Reported') +`</br>`+`<b>ID: </b>` +  (facility.attributes.Fac_PWSID ? facility.attributes.Fac_PWSID : 'Not Reported') + '</p>' +`<p style="text-align: center;">&nbsp;</p> <table style="height: 98px; background-color: #d5e1ef; border-color: #000000; margin-left: auto; margin-right: auto;" width="100%"><tbody><tr><td style="text-align: center; width: 287px;"><b><u>PWS Contact Information</u></b>`+`<div id="admincontacts"></div></td></tr>`+`</br></tbody></table><p>&nbsp;</p>`+`</br>`+`<div id="pwsinfo"></div>`+`<p style="text-align: center;"><a href="https://echo.epa.gov/detailed-facility-report?fid=${facility.attributes.Fac_PWSID}" target="_blank"><b>ECHO Detailed System Report</b> </a></p>`+`</br>`+`<b><p style="text-align: center;">Water System Facility Information</p></b>` + `<hr />`+ `<b>Facility Name:</b>` + ` `+ (facility.attributes.FacilityName ? facility.attributes.FacilityName : 'Not Reported') + '</br>' + `<b>Facility ID: </b>`+(facility.attributes.FacilityID ? facility.attributes.FacilityID : 'Not Reported')+`</br>`+ `<b>Facility Type: </b>` + (facilitytype ? facilitytype : 'Not Reported') + '</br>' + `<b>Source Type:</b>` + ` `+  (sourcetype ? sourcetype : 'Not Reported') +`</br>`+ `<b>Source Treated: </b>`+ (trtstatus ? trtstatus :'Not Reported') + `</br>`+ `<b>Facility Availability:</b>`+ ` `+ (availability ? availability : 'Not Reported') +`</br>` + `<b>Last Updated:</b>` +` `+ (facility.attributes.Last_Reported ? facility.attributes.Last_Reported : 'Not Reported') +`</br>`+'<b>PWS Purchased From: </b>'+ (facility.attributes.PWSID_SELLER ? facility.attributes.PWSID_SELLER : 'Not Reported') + `</br>` + `<b>Purchased Water Treated: </b>`+ (sellertreated ? sellertreated : 'Not Reported')+`</br>`+`</br>`+`</br>`+`<table style="height: 98px; background-color: #acc3df; border-color: #000000; margin-left: auto; margin-right: auto;" width="100%"><tbody><tr><td style="text-align: center; width: 287px;"><strong><p style="text-align: center;"><u>Regulatory Agency</u></strong>`+`</br>`+`<div id="tableinfo"></div>`+`</tbody></table></p></td></tr><p>&nbsp;</p>`;
    this.loadingShelter.hide();
