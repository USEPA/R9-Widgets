...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/_base/config', 'dojo/Evented', 'dijit/_WidgetBase', 'dojo/when', 'dojo/promise/all', 'dojo/Deferred', 'esri/request', 'moment/moment', 'dojo/date/locale', 'dojo/i18n', 'esri/lang', 'esri/TimeExtent'], function (declare, lang, array, dojoConfig, Evented, _WidgetBase, when, all, Deferred, esriRequest, moment, dateLocale, i18n, esriLang, TimeExtent) {
  ...
  var clazz = declare([_WidgetBase, Evented], {
    ...
    _getUpdatedFullTime: function _getUpdatedFullTime() {
      var i = 0,
          len,
          layer,
          layerId;
    ...
    _getFullTimeExtent: function _getFullTimeExtent(timeExtents) {
      ...
      array.forEach(timeExtents, lang.hitch(this, function (te) {
        ...
        if (!fullTimeExtent) {
          fullTimeExtent = new TimeExtent(new Date(te.startTime.getTime()), new Date(te.endTime.getTime()));
        } else {
      ...
      fullTimeExtent.endTime = new Date(fullTimeExtent.endTime.getFullYear(), fullTimeExtent.endTime.getMonth(), fullTimeExtent.endTime.getDate(), fullTimeExtent.endTime.getHours(), fullTimeExtent.endTime.getMinutes() + 1, 0, 0);

      return fullTimeExtent;
    },
    //update liveData timeExtent
    _getUpdatedTime: function _getUpdatedTime(layer) {
    ...
    getUpdatedFullTime: function getUpdatedFullTime() {
      var i = 0,
          len,
          layer,
          layerId;
    ...
    needUpdateFullTime: function needUpdateFullTime() {
      var i = 0,
          len,
          layer,
          layerId;
    ...
    hasLiveData: function hasLiveData(layer) {
    ...
    _getTimeFormatLabel: function _getTimeFormatLabel(timeExtent) {
      ...
      if (this.config.timeFormat !== "auto" && !("Custom" === this.config.timeFormat && "" === this.config.customDateFormat)) {
        ...
        if (timePattern) {
          ...
          var startTime3 = i18n.getLocalization("dojo.cldr", "gregorian")["dateTimeFormat-medium"].replace(/\{1\}/g, startTime).replace(/\{0\}/g, startTime2);
        ...
        if (end) {
          ...
          if (timePattern) {
            ...
            if (showEndDate && timePattern) {
              var endTime3 = i18n.getLocalization("dojo.cldr", "gregorian")["dateTimeFormat-medium"].replace(/\{1\}/g, endTime).replace(/\{0\}/g, endTime2);
              endTime = endTime3;
            } else {
              endTime = endTime2;
            }
    ...
    findDefaultInterval: function findDefaultInterval(fullTimeExtent) {
      var interval;
      var units;
    ...
    getPropsFromMultiLayerInfos: function getPropsFromMultiLayerInfos(timeInfos, ref) {
      var res = {};
      if (null === ref) {
        res = timeInfos;
        res.thumbCount = 2;
    ...
    getTsPros: function getTsPros() {
      ...
      var timeSliderProps = lang.getObject('itemData.widgets.timeSlider.properties', false, itemInfo);
    ...
    findClosestThumbIndex: function findClosestThumbIndex(timeSlider, timeValue) {
