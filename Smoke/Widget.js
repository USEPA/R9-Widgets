      ...
      vm.legend_update_interval = setInterval(function () {
        ...
        if (vm._legend.domNode.children.length === 2) {
          if (vm._legend.domNode.innerHTML.indexOf('id="smoke_widget_legend"') === -1) {
            vm.wind_legend = domConstruct.toDom('<div style="display: block;" class="esriLegendService" id="smoke_widget_legend">' + '<table><tbody>' + '<tr><td align="left" colspan="2"><span class="esriLegendServiceLabel">Smoke Predictions*<br/>' + vm.start.clone().add(1, 'hour').format('ll hA') + ' to ' + vm.start.clone().add(69, 'hour').format('ll hA') + '</span></td></tr>' + '<tr><td colspan="2"><img style="max-width: 100%" src="https://r9.ercloud.org/r9wab/smoke_data/' + vm.start.format('YYYYMMDDHH') + '/legend.png"/>' + '<tr><td colspan="2">*Experimental Research Output provided by USFS R&D. Use at your own Risk. <a target="_blank" href="https://info.airfire.org/daily-run-viewer">More Info</a> ' + '</td></tr>' + '</tbody></table></div>');
