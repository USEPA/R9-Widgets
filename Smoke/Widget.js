...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/array', 'dojo/_base/fx',
  ...
  function (declare, lang, html, array, baseFx, WidgetManager, Move, on, query, Deferred, domGeometry,
            LayerInfos, BaseWidget, TimeExtent, TimeSlider, SpeedMenu, jimuUtils, TimeProcesser, utils,
            throttle, moment, request, MapImageLayer, MapImage, string, PanelManager, domConstruct) {
    ...
    var clazz = declare([BaseWidget], {
      ...
      _getImages: function _getImages(start_timestamp, image_timestamp) {
        ...
        if (image_timestamp === undefined) {
          ...
          return request(string.substitute(vm.root_url, {
            start_timestamp: start_timestamp.format('YYYYMMDDHH'),
      ...
      postCreate: function postCreate() {
        ...
        this.start = -moment().startOf('day').diff(moment(), 'hours') > 12 ? moment().set({
          hours: 12,
          minutes: '00'
        }) : moment().set({hours: '00', minutes: '00'});
      ...
      createTimeSlider: function createTimeSlider() {
        ...
        this.timeSlider.on('time-extent-change', function (timeExtent) {
          ...
          vm.mapImageLayer.addImage(vm.images[moment(timeExtent.endTime).format('YYYYMMDDHHmm')]);
          var label = moment(timeExtent.endTime).format('ll hA');
        ...
        vm.legend_update_interval = setInterval(function () {
          ...
          if (vm._legend.domNode.children.length === 2) {
            if (vm._legend.domNode.innerHTML.indexOf('id="smoke_widget_legend"') === -1) {
              vm.wind_legend = domConstruct.toDom('<div style="display: block;" class="esriLegendService" id="smoke_widget_legend">' +
                '<table><tbody>' +
                '<tr><td align="left" colspan="2"><span class="esriLegendServiceLabel">Smoke Predictions*<br/>' +
                vm.start.clone().add(1, 'hour').format('ll hA') + ' to ' + vm.start.clone().add(69, 'hour').format('ll hA') +
                '</span></td></tr>' +
                '<tr><td colspan="2"><img style="max-width: 100%" src="https://r9.ercloud.org/r9wab/smoke_data/' + vm.start.format('YYYYMMDDHH') + '/legend.png"/>' +
                '<tr><td colspan="2">*Experimental Research Output provided by USFS R&D. Use at your own Risk. <a target="_blank" href="https://info.airfire.org/daily-run-viewer">More Info</a> ' +
                '</td></tr>' +
                '</tbody></table></div>');
