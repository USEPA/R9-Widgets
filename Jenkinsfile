...
try {
  ...
  stage('build') {
    ...
    slackSend(channel:"#r9-service-alerts", message: "COP/Widget Branch ${env.BRANCH_NAME} Build FAILED")
  } finally {
    cleanWs()
  }
  try {
  stage('build') {
    ...
    slackSend(channel:"#r9-service-alerts", message: "Firemap/Widget Branch ${env.BRANCH_NAME} Build FAILED")
  } finally {
    cleanWs()
  }
}
